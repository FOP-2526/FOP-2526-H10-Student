...
public interface Enclosure<TODO_REPLACE> {
    ...
    default Location getMostCrowdedLocation() {
        // TODO: H10.2.1
        return org.tudalgo.algoutils.student.Student.crash("H10.2.1 - Remove if implemented");
    }
    ...
    default boolean anyMatch(Predicate predicate) {
        for (int i = 0; i < this.getAnimals().size(); i++) {
            Object animal = this.getAnimals().get(i);
    ...
    default void mapObjectOriented(Function mapper) {
        for (int i = 0; i < this.getAnimals().size(); i++) {
            Object animal = getAnimals().get(i);
            Object mappedAnimal = mapper.apply(animal);
    ...
    default Object reduce(Object identity, BiFunction accumulator) {
    ...
    default Enclosure mapFunctional(Supplier<Enclosure> supp, Function mapper) {
        Enclosure result = supp.get();
        for (int i = 0; i < this.getAnimals().size(); i++) {
            Object animal = this.getAnimals().get(i);
            Object mappedAnimal = mapper.apply(animal);
    ...
    BiFunction SIGN_WITH_ANIMAL_NAME = null;
    ...
    static Consumer flyAndEat() {
        // TODO: H10.4.3
        return org.tudalgo.algoutils.student.Student.crash("H10.4.3 - Remove if implemented");
    }
